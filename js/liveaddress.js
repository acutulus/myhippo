...
liveaddress.run(['$templateCache', function($templateCache) {
  $templateCache.put('angular-liveaddress.html',
    '<div class="liveaddress">'+
      '<input type="text" ng-model="address" class="{{ inputClass }}" placeholder="{{ inputPlaceholder }}"ng-class="{current: suggestions.length}" ng-keydown="handleKeydown($event)" ng-keyup="handleKeyup($event)" ng-blur="handleBlur()"/>'+
